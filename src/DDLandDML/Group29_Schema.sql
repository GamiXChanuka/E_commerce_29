...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `DropOrder`(IN orderId INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCartItems`(IN cartID INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetOrdersByUser`(IN userID INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetProductAttributes`(IN variantID INT)
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `GetVariantDetails` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetVariantDetails`(IN inputVariantID INT)
BEGIN
    -- Fetch variant details and related image link
    SELECT v.VariantID, v.VariantName, v.Price, i.ImageLink
    FROM Variant v
    LEFT JOIN image i ON v.VariantID = i.VariantID
    WHERE v.VariantID = inputVariantID;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    ...
    DECLARE orderId INT; -- Declare orderId at the beginning

    -- Start the transaction
    START TRANSACTION;

    -- Check if the cart has any items for the user
    SELECT COUNT(*) INTO cartItemsCount
    FROM CartItem ci
    ...
    IF cartItemsCount = 0 THEN
        ...
        SET MESSAGE_TEXT = 'Cart is empty, cannot proceed with order.';
    END IF;
    ...
    IF totalPrice <= 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Total price is zero, cannot proceed with order.';
    END IF;

    -- Insert the order
    INSERT INTO `Order` (UserID, OrderDate, DeliveryType, PaymentMethod, OrderTotal, AddressID)
    VALUES (userId, CURRENT_DATE, deliveryType, paymentMethod, totalPrice, addressId);

    -- Get the last inserted OrderID
    SET orderId = LAST_INSERT_ID();

    -- Insert each item from the cart into OrderItem
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SetCart` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SetCart`(
    IN operation VARCHAR(10), -- 'add' or 'remove'
    IN variantID INT,
    IN cartID INT
)
BEGIN
    DECLARE variantExists INT DEFAULT 0;
    DECLARE cartItemExists INT DEFAULT 0;
    DECLARE currentQuantity INT DEFAULT 0;

    -- Check if the variant exists
    SELECT COUNT(*) INTO variantExists
    FROM Variant
    WHERE VariantID = variantID;

    IF variantExists = 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Product variant not found';
    END IF;

    -- Check if the variant is already in the cart
    SELECT COUNT(*), Quantity INTO cartItemExists, currentQuantity
    FROM CartItem
    WHERE VariantID = variantID AND CartID = cartID;

    IF operation = 'add' THEN
        IF cartItemExists = 0 THEN
            -- If the product variant is not in the cart, add it with quantity 1
            INSERT INTO CartItem (VariantID, CartID, Quantity)
            VALUES (variantID, cartID, 1);
        ELSE
    ...
    ELSEIF operation = 'remove' THEN
        IF cartItemExists = 0 THEN
            SIGNAL SQLSTATE '45000' 
            SET MESSAGE_TEXT = 'Product variant not in the cart';
        ELSE
            -- If the current quantity is more than 1, decrease the quantity
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `UnregisterSetCart` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `UnregisterSetCart`(
    IN inputVariantID INT,
    IN inputCartID INT,
...
BEGIN
    ...
    INSERT INTO CartItem (VariantID, CartID, Quantity)
    VALUES (inputVariantID, inputCartID, inputQuantity);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-10-15 20:44:20
